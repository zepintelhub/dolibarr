...
/**
 *   	\file       echeancepaiement_list.php
 *		\ingroup    caissealimentation
 *		\brief      List page for echeancepaiement
 */
...
require_once __DIR__.'/class/echeancepaiement.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('caissealimentation', 'echeancepaiement', 'write');
	$permissiontodelete = $user->hasRight('caissealimentation', 'echeancepaiement', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("caissealimentation")) {
	accessforbidden('Module caissealimentation not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'EcheancePaiement';
	$objectlabel = 'EcheancePaiement';
	$uploaddir = $conf->caissealimentation->dir_output;
...
$title = $langs->trans("EcheancePaiements");
//$help_url = "EN:Module_EcheancePaiement|FR:Module_EcheancePaiement_FR|ES:Módulo_EcheancePaiement";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/caissealimentation/echeancepaiement_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-caissealimentation page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/caissealimentation/echeancepaiement_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEcheancePaiementRef";
$modelmail = "echeancepaiement";
$objecttmp = new EcheancePaiement($db);
