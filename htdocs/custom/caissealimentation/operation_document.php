...
/**
 *  \file       operation_document.php
 *  \ingroup    caissealimentation
 *  \brief      Tab for documents linked to Operation
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->caissealimentation->multidir_output[$object->entity ? $object->entity : $conf->entity]."/operation/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('caissealimentation', 'operation', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Operation")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Operation"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='operation/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'operation/'.dol_sanitizeFileName($object->ref).'/';
