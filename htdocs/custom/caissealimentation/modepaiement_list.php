...
/**
 *   	\file       modepaiement_list.php
 *		\ingroup    caissealimentation
 *		\brief      List page for modepaiement
 */
...
require_once __DIR__.'/class/modepaiement.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('caissealimentation', 'modepaiement', 'write');
	$permissiontodelete = $user->hasRight('caissealimentation', 'modepaiement', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ModePaiement';
	$objectlabel = 'ModePaiement';
...
$title = $langs->trans("ModePaiements");
//$help_url = "EN:Module_ModePaiement|FR:Module_ModePaiement_FR|ES:Módulo_ModePaiement";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/caissealimentation/modepaiement_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/caissealimentation/modepaiement_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendModePaiementRef";
$modelmail = "modepaiement";
$objecttmp = new ModePaiement($db);
