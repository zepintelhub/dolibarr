...
/**
 *   	\file       leveedesfonds_list.php
 *		\ingroup    gestionnaire
 *		\brief      List page for leveedesfonds
 */
...
require_once __DIR__.'/class/leveedesfonds.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('gestionnaire', 'leveedesfonds', 'write');
	$permissiontodelete = $user->hasRight('gestionnaire', 'leveedesfonds', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("gestionnaire")) {
	accessforbidden('Module gestionnaire not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'LeveeDesFonds';
	$objectlabel = 'LeveeDesFonds';
	$uploaddir = $conf->gestionnaire->dir_output;
...
$title = $langs->trans("LeveeDesFondss");
//$help_url = "EN:Module_LeveeDesFonds|FR:Module_LeveeDesFonds_FR|ES:Módulo_LeveeDesFonds";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/gestionnaire/leveedesfonds_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-gestionnaire page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/gestionnaire/leveedesfonds_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendLeveeDesFondsRef";
$modelmail = "leveedesfonds";
$objecttmp = new LeveeDesFonds($db);
