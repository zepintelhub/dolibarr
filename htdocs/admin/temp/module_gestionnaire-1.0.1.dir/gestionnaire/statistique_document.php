...
/**
 *  \file       statistique_document.php
 *  \ingroup    gestionnaire
 *  \brief      Tab for documents linked to Statistique
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->gestionnaire->multidir_output[$object->entity ? $object->entity : $conf->entity]."/statistique/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('gestionnaire', 'statistique', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Statistique")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Statistique"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='statistique/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'statistique/'.dol_sanitizeFileName($object->ref).'/';
