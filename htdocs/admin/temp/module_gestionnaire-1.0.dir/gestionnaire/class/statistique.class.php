...
/**
 * \file        class/statistique.class.php
 * \ingroup     gestionnaire
 * \brief       This file is a CRUD class file for Statistique (Create/Read/Update/Delete)
 */
...
/**
 * Class for Statistique
 */
class Statistique extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'statistique';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'gestionnaire_statistique';
	...
	/**
	 * @var string String with name of icon for statistique. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'statistique@gestionnaire' if picto is file 'img/object_statistique.png'.
	 */
	...
	//  */
	// public $table_element_line = 'gestionnaire_statistiqueline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_statistique';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Statistiqueline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Statistique', 'fk_element'=>'fk_statistique'));
	...
	//  */
	// protected $childtablesoncascade = array('gestionnaire_statistiquedet');

	// /**
	//  * @var StatistiqueLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('gestionnaire', 'statistique', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnaire', 'statistique', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnaire', 'statistique_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'statistique/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'statistique/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'statistique/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'statistique/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->gestionnaire->dir_output.'/statistique/'.$oldref;
				$dirdest = $conf->gestionnaire->dir_output.'/statistique/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gestionnaire->dir_output.'/statistique/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowStatistique")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Statistique").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/gestionnaire/statistique_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowStatistique");
	...
	public function getLinesArray()
	{
		...
		$objectline = new StatistiqueLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_statistique = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('GESTIONNAIRE_MYOBJECT_ADDON')) {
			$conf->global->GESTIONNAIRE_MYOBJECT_ADDON = 'mod_statistique_standard';
		}
...
/**
 * Class StatistiqueLine. You can also remove this and generate a CRUD class for lines objects.
 */
class StatistiqueLine extends CommonObjectLine
{
	// To complete with content of an object StatistiqueLine
	// We should have a field rowid, fk_statistique and position
